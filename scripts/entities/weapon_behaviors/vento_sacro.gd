static func fire(w, weapon_manager, player, get_enemies):
	...
	var dmg_mult = 1.0 + speed / 400.0
	if player._crit_chance > 0 and randf() < player._crit_chance:
		dmg_mult *= player._crit_mult
	var spread = PI / 6
	for i in range(count):
		var offset_angle = (i - (count - 1) / 2.0) * 0.15
		...
		p.body_entered.connect(weapon_manager._on_proj_hit_and_free.bind(p, dmg * dmg_mult))
